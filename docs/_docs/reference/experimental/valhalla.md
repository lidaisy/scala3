...
Valhalla value classes are the Scala equivalence of Java's Project Valhalla's value classes. Valhalla value classes differ from the current Scala value classes by allowing more than one field. Valhalla value classes behave almost the same as regular classes, except that they do not have identity. Two valhalla value classes are equal when their fields are the same.

...
Valhalla value classes are the Scala equivalence of Java's Project Valhalla's value classes. Valhalla value classes differ from the current Scala value classes by allowing more than one field. Valhalla value classes behave almost the same as regular classes, except that they do not have identity. Two valhalla value classes are equal when the fields are the same.
...
Valhalla value classes are implicitly final and cannot be extended unless it is abstract. Its fields are immutable.
...
Similar to Valhalla value classes, any Valhalla trait must have immutable fields only.
